  ...
  try {
    if (fs.existsSync(LEARNING_FILE)) {
      ...
      for (let type of ['hu', 'md5']) {
        if (data[type]) learningData[type] = { ...learningData[type], ...data[type] };
      }
...
// === TRANSFORM DATA (FIX CHO API ĐÚNG CẤU TRÚC) ===
function transformApiData(apiData) {
  if (!apiData) return [];
  let items = [];
  if (apiData.data && Array.isArray(apiData.data)) items = apiData.data;
  else if (Array.isArray(apiData)) items = apiData;
  else if (apiData.list && Array.isArray(apiData.list)) items = apiData.list;
  else return [];
  
  return items.map(item => ({
    Phien: item.phien || item.Phien || item.id || 0,
    Ket_qua: (item.ket_qua === 'TAI' || item.Ket_qua === 'Tài' || item.resultTruyenThong === 'TAI') ? 'Tài' : 'Xỉu',
    Tong: item.point || item.Tong || item.tong || 0
  }));
}
...
function superAdvancedPrediction(data, type) {
  ...
  if (markovPred) {
    if (markovPred.prediction === 'Tài') taiScore += markovPred.confidence * 1.4;
    else xiuScore += markovPred.confidence * 1.4;
  ...
  if (sumTrend) {
    if (sumTrend.prediction === 'Tài') taiScore += sumTrend.confidence * 1.3;
    else xiuScore += sumTrend.confidence * 1.3;
...
app.get('/hu/lichsu', (req, res) => {
  res.json({ history: predictionHistory.hu.slice(0, 50), total: predictionHistory.hu.length });
});

app.get('/md5/lichsu', (req, res) => {
  res.json({ history: predictionHistory.md5.slice(0, 50), total: predictionHistory.md5.length });
});
...
async function autoProcessPredictions() {
  try {
    ...
    if (dataHu && dataHu.length > 0 && lastProcessedPhien.hu !== dataHu[0].Phien + 1) {
      const result = superAdvancedPrediction(dataHu, 'hu');
      savePredictionToHistory('hu', dataHu[0].Phien + 1, result.prediction, result.confidence, dataHu[0]);
      lastProcessedPhien.hu = dataHu[0].Phien + 1;
      console.log(`[Auto] Hu: ${result.prediction} (${result.confidence}%)`);
    }
    
    const dataMd5 = await fetchDataMd5();
    if (dataMd5 && dataMd5.length > 0 && lastProcessedPhien.md5 !== dataMd5[0].Phien + 1) {
      const result = superAdvancedPrediction(dataMd5, 'md5');
      savePredictionToHistory('md5', dataMd5[0].Phien + 1, result.prediction, result.confidence, dataMd5[0]);
      lastProcessedPhien.md5 = dataMd5[0].Phien + 1;
