...
class TaiXiuAnalyzer {
        ...
        switch (model.logic) {
            ...
            case 'recovery':
                ...
                if (last4b[0] === last4b[1] && last4b[1] !== last4b[2] && last4b[2] !== last4b[3]) {
                    return { prediction: last4b[3] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Cầu 1-1 đang phục hồi sau gãy' };
                }
                break;
        }
        return null;
    ...
    runSubModelBreak(results, model) {
        ...
        switch (model.logic) {
            ...
            case '11to22':
                if (last6.length === 6 && last6[0] !== last6[1] && last6[1] !== last6[2] && last6[2] === last6[3] && last6[3] !== last6[4] && last6[4] === last6[5]) {
                    return { prediction: last6[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Chuyển từ cầu 1-1 sang 2-2' };
                }
                break;
            case '22to11':
                if (last6.length === 6 && last6[0] === last6[1] && last6[1] !== last6[2] && last6[2] !== last6[3] && last6[3] !== last6[4] && last6[4] !== last6[5]) {
                    return { prediction: last6[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Chuyển từ cầu 2-2 sang 1-1' };
                }
                break;
            case 'streakTo11':
                if (last5.length === 5 && last5[0] === last5[1] && last5[1] === last5[2] && last5[2] !== last5[3] && last5[3] !== last5[4]) {
                    return { prediction: last5[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Chuyển từ bệt sang cầu 1-1' };
                }
                break;
        }
        return null;
    ...
    analyzeDiceVolatility(history) {
        ...
        return { prediction: lastResult, confidence: 0.4, reason: "Không phát hiện biến động đặc biệt" };
    }
    ...
    getMostCommon(counts) {
        if (counts['Tài'] >= counts['Xỉu']) return { value: 'Tài', count: counts['Tài'] };
        else return { value: 'Xỉu', count: counts['Xỉu'] };
    }
...
async function updateHistoryFromAPI() {
    try {
        ...
        for (const item of apiData) {
            const id = item.phien || item.session || item.id;
            const existing = resultHistory.find(h => h.phien === id);
            if (!existing) {
        ...
        if (newEntries > 0) {
            ...
            fs.writeFileSync(HISTORY_FILE, JSON.stringify(resultHistory, null, 2));
            console.log(`[📥] Đã thêm ${newEntries} phiên mới từ API. Tổng: ${resultHistory.length}`);
        }
        ...
        if (resultHistory.length >= 5) {
            ...
            const lastSession = resultHistory[resultHistory.length - 1];
            const nextPhien = lastSession.phien ? parseInt(lastSession.phien) + 1 : (resultHistory.length + 1);
            ...
            console.log(`\n🎲 Phiên cuối: ${lastSession.phien} | KQ: ${lastSession.Ket_qua}`);
            console.log(`🤖 Dự đoán phiên ${nextPhien}: ${finalPrediction} (${(finalConfidence * 100).toFixed(0)}%)`);
            console.log(`🔍 Phát hiện: ${finalType}`);
            console.log(`📊 ${ensembleResult.details.length} models tham gia\n`);
        }
    } catch (e) {
        console.error('[❌] Lỗi cập nhật lịch sử:', e.message);
    }
}
...
app.listen(PORT, async () => {
    ...
    console.log(`[🔄] Đang tải lịch sử từ API...`);
    
    await updateHistoryFromAPI();
    
    // Tự động cập nhật mỗi 10 giây
    setInterval(updateHistoryFromAPI, 10000);
});