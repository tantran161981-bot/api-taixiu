...
// ==================== TAI XIU ANALYZER (GIỮ NGUYÊN 100%) ====================
class TaiXiuAnalyzer {
    ...
    superAnalysis(results) {
        ...
        if (score >= 2) {
            ...
            if (taiWeight > xiuWeight * 1.5) {
                return { prediction: 'Tài', confidence: 0.85, reason: 'Siêu phân tích đồng thuận Tài' };
            } else if (xiuWeight > taiWeight * 1.5) {
                return { prediction: 'Xỉu', confidence: 0.85, reason: 'Siêu phân tích đồng thuận Xỉu' };
            }
        }
        return { confidence: 0 };
    }
        ...
        switch (model.logic) {
            case 'pure':
                if (this.isPerfectAlternating(results, 4)) {
                    return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.9, reason: 'Phát hiện cầu 1-1 thuần túy', model_name: model.name };
                }
                break;
            case 'variant':
                if (this.isAlternatingWithTolerance(results, 1)) {
                    return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8, reason: 'Phát hiện cầu 1-1 biến thể', model_name: model.name };
                }
                break;
            case 'long':
                ...
                if (altCount >= 8) {
                    return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7 + (altCount / 20), reason: `Cầu 1-1 dài hạn với ${altCount}/11 cặp xen kẽ`, model_name: model.name };
                }
                break;
            case 'hybrid':
                const recent = results.slice(-5);
                if (recent[0] !== recent[1] && recent[1] !== recent[2] && recent[3] !== recent[4]) {
                    return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Phát hiện cầu 1-1 kết hợp', model_name: model.name };
                }
                break;
            case 'break':
                if (last4[0] !== last4[1] && last4[1] !== last4[2] && last4[2] !== last4[3]) {
                    ...
                    if (streak > 4) {
                        return { prediction: last, confidence: 0.8, reason: 'Cầu 1-1 dài sắp gãy, dự đoán giữ nguyên', model_name: model.name };
                    }
                }
                break;
            case 'recovery':
                if (last4[0] === last4[1] && last4[1] !== last4[2] && last4[2] !== last4[3]) {
                    return { prediction: last4[3] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Cầu 1-1 đang phục hồi sau gãy', model_name: model.name };
                }
                break;
        }
        return null;
        ...
        switch (model.logic) {
            case 'pure':
                if (last6.length === 6 && last6[0] === last6[1] && last6[1] !== last6[2] && last6[2] === last6[3] && last6[3] !== last6[4] && last6[4] === last6[5]) {
                    return { prediction: last6[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.9, reason: 'Phát hiện cầu 2-2 chuẩn', model_name: model.name };
                }
                break;
            case 'offset':
                if (last6.length === 6 && last6[0] === last6[1] && last6[1] !== last6[2] && last6[2] !== last6[3] && last6[3] === last6[4] && last6[4] !== last6[5]) {
                    return { prediction: last6[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8, reason: 'Phát hiện cầu 2-2 lệch', model_name: model.name };
                }
                break;
            case 'variant':
                if (last8.length === 8 && last8[0] === last8[1] && last8[1] !== last8[2] && last8[2] === last8[3] && last8[3] !== last8[4] && last8[4] === last8[5] && last8[5] !== last8[6] && last8[6] === last8[7]) {
                    return { prediction: last8[6] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Phát hiện cầu 2-2 biến tướng', model_name: model.name };
                }
                break;
            case 'hybrid':
                if (last6.length === 6 && last6[0] === last6[1] && last6[1] !== last6[2] && last6[2] !== last6[3] && last6[3] !== last6[4] && last6[4] === last6[5]) {
                    return { prediction: last6[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Cầu 2-2 kết hợp 1-1', model_name: model.name };
                }
                break;
            case 'long':
                if (last8.length === 8) {
                    ...
                    if (score >= 3) {
                        return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7 + (score * 0.05), reason: `Cầu 2-2 dài với ${score}/4 cặp đúng`, model_name: model.name };
                    }
                }
                break;
            case 'break':
                if (last6.length === 6 && last6[0] === last6[1] && last6[1] !== last6[2] && last6[2] === last6[3] && last6[3] !== last6[4] && last6[4] !== last6[5]) {
                    return { prediction: last6[4], confidence: 0.85, reason: 'Phát hiện bẻ cầu 2-2', model_name: model.name };
                }
                break;
        }
        return null;
    ...
    runSubModelStreak(results, model) {
        ...
        switch (model.logic) {
            case 'short':
                if (streak >= 2 && streak <= 3) return { prediction: last, confidence: 0.7 + (streak * 0.05), reason: `Bệt ngắn ${streak} phiên`, model_name: model.name };
            case 'medium':
                if (streak >= 4 && streak <= 5) return { prediction: last, confidence: 0.75 + ((streak - 4) * 0.05), reason: `Bệt trung ${streak} phiên`, model_name: model.name };
            case 'long':
                if (streak >= 6) return { prediction: last, confidence: 0.8 + (Math.min(streak, 10) * 0.01), reason: `Bệt dài ${streak} phiên`, model_name: model.name };
            case 'break':
                if (streak >= 4) return { prediction: other, confidence: 0.6 + (streak * 0.03), reason: `Bệt ${streak} phiên, dự đoán sắp gãy`, model_name: model.name };
            case 'hybrid':
                if (streak >= 3) {
                    const prev = results[results.length - streak - 1];
                    if (prev && prev !== last) return { prediction: last, confidence: 0.7, reason: `Bệt sau khi đảo từ ${prev}`, model_name: model.name };
                }
                break;
            case 'super':
                if (streak >= 8) return { prediction: last, confidence: 0.9, reason: `Siêu bệt ${streak} phiên`, model_name: model.name };
        }
        return null;
    }
        ...
        switch (model.logic) {
            case 'pure':
                if (last9.length === 9 && last9[0] === last9[1] && last9[1] === last9[2] && last9[3] === last9[4] && last9[4] === last9[5] && last9[6] === last9[7] && last9[7] === last9[8] && last9[0] !== last9[3] && last9[3] !== last9[6]) {
                    return { prediction: last9[6] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.9, reason: 'Phát hiện cầu 3-3 chuẩn', model_name: model.name };
                }
                break;
            case 'variant':
                if (last12.length === 12) {
                    ...
                    if (score >= 3) {
                        return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7 + (score * 0.05), reason: `Cầu 3-3 biến thể với ${score}/4 bộ ba`, model_name: model.name };
                    }
                }
                break;
            case 'short':
                ...
                if (last6[0] === last6[1] && last6[1] === last6[2] && last6[3] === last6[4] && last6[4] === last6[5]) {
                    return { prediction: last6[3] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Cầu 3-3 ngắn (6 phiên)', model_name: model.name };
                }
                break;
            case 'hybrid':
                if (last9.length === 9 && last9[0] === last9[1] && last9[1] === last9[2] && last9[3] !== last9[4] && last9[5] === last9[6] && last9[6] === last9[7]) {
                    return { prediction: last9[6] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Cầu 3-3 kết hợp', model_name: model.name };
                }
                break;
            case 'break':
                if (last9.length === 9 && last9[0] === last9[1] && last9[1] === last9[2] && last9[3] === last9[4] && last9[4] === last9[5] && last9[6] !== last9[7]) {
                    return { prediction: last9[6], confidence: 0.8, reason: 'Phát hiện bẻ cầu 3-3', model_name: model.name };
                }
                break;
            case 'long':
                if (results.length >= 15) {
                    ...
                    if (pattern.length >= 4 && pattern[0] !== pattern[1] && pattern[1] !== pattern[2]) {
                        return { prediction: pattern[pattern.length-1] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8, reason: 'Cầu 3-3 dài hạn', model_name: model.name };
                    }
                }
                break;
        }
        ...
        switch (model.logic) {
            case 'pure':
                if (last5.length === 5 && last5[0] === last5[1] && last5[1] !== last5[2] && last5[2] !== last5[3] && last5[3] === last5[4] && last5[0] === last5[3]) {
                    return { prediction: last5[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.9, reason: 'Phát hiện cầu 2-1-2 chuẩn', model_name: model.name };
                }
                break;
            case 'variant':
                if (last7.length === 7 && last7[0] === last7[1] && last7[1] !== last7[2] && last7[3] === last7[4] && last7[4] !== last7[5] && last7[0] === last7[3]) {
                    return { prediction: last7[5] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8, reason: 'Phát hiện cầu 2-1-2 biến thể', model_name: model.name };
                }
                break;
            case 'long':
                if (results.length >= 10) {
                    ...
                    if (count >= 2) {
                        return { prediction: results[results.length-1] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Cầu 2-1-2 dài hạn', model_name: model.name };
                    }
                }
                break;
        }
    ...
    runSubModel121(results, model) {
        ...
        switch (model.logic) {
            case 'pure':
                if (last5.length === 5 && last5[0] !== last5[1] && last5[1] === last5[2] && last5[2] !== last5[3] && last5[3] === last5[4] && last5[0] === last5[3]) {
                    return { prediction: last5[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.9, reason: 'Phát hiện cầu 1-2-1 chuẩn', model_name: model.name };
                }
                break;
            case 'variant':
                if (last7.length === 7 && last7[0] !== last7[1] && last7[1] === last7[2] && last7[3] !== last7[4] && last7[4] === last7[5] && last7[0] === last7[3]) {
                    return { prediction: last7[5] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8, reason: 'Phát hiện cầu 1-2-1 biến thể', model_name: model.name };
                }
                break;
            case 'long':
                if (results.length >= 10) {
                    ...
                    for (let i = 0; i < 5; i+=2) if (i+4 < 10 && last10[i] !== last10[i+1] && last10[i+1] === last10[i+2] && last10[i+3] === last10[i+4]) count++;
                    if (count >= 2) {
                        return { prediction: results[results.length-1] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Cầu 1-2-1 dài hạn', model_name: model.name };
                    }
                }
                break;
        }
        ...
        switch (model.logic) {
            case 'break11':
                if (last4.length === 4 && last4[0] !== last4[1] && last4[1] !== last4[2] && last4[2] === last4[3]) {
                    return { prediction: last4[3], confidence: 0.85, reason: 'Phát hiện bẻ cầu 1-1', model_name: model.name };
                }
                break;
            case 'break22':
                if (last5.length === 5 && last5[0] === last5[1] && last5[1] !== last5[2] && last5[2] === last5[3] && last5[3] !== last5[4] && last5[0] === last5[4]) {
                    return { prediction: last5[4], confidence: 0.85, reason: 'Phát hiện bẻ cầu 2-2', model_name: model.name };
                }
                break;
            case 'breakStreak':
                const streak = this.getStreak(results.slice(0, -1));
                if (streak >= 3 && last !== results[results.length - 2]) {
                    return { prediction: last, confidence: 0.8, reason: `Phát hiện bẻ cầu bệt sau ${streak} phiên`, model_name: model.name };
                }
                break;
            case '11to22':
                if (last6.length === 6 && last6[0] !== last6[1] && last6[1] !== last6[2] && last6[2] === last6[3] && last6[3] !== last6[4] && last6[4] === last6[5]) {
                    return { prediction: last6[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Chuyển từ cầu 1-1 sang 2-2', model_name: model.name };
                }
                break;
            case '22to11':
                if (last6.length === 6 && last6[0] === last6[1] && last6[1] !== last6[2] && last6[2] !== last6[3] && last6[3] !== last6[4] && last6[4] !== last6[5]) {
                    return { prediction: last6[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Chuyển từ cầu 2-2 sang 1-1', model_name: model.name };
                }
                break;
            case 'streakTo11':
                if (last5.length === 5 && last5[0] === last5[1] && last5[1] === last5[2] && last5[2] !== last5[3] && last5[3] !== last5[4]) {
                    return { prediction: last5[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Chuyển từ bệt sang cầu 1-1', model_name: model.name };
                }
                break;
        }
        return null;
        ...
        switch (model.logic) {
            case 'frequency':
                ...
                if (freq.dominant && freq.ratio > 0.6) {
                    return { prediction: freq.dominant, confidence: 0.6 + (freq.ratio * 0.2), reason: `Tần suất ${freq.dominant} chiếm ${(freq.ratio*100).toFixed(0)}%`, model_name: model.name };
                }
                break;
            case 'cycle':
                const cycle = this.detectCycle(results);
                if (cycle.found) {
                    return { prediction: cycle.next, confidence: 0.7, reason: `Phát hiện chu kỳ ${cycle.length} phiên`, model_name: model.name };
                }
                break;
            case 'symmetry':
                const symmetry = this.checkSymmetry(results);
                if (symmetry.found) {
                    return { prediction: symmetry.prediction, confidence: 0.75, reason: 'Phát hiện cầu đối xứng', model_name: model.name };
                }
                break;
            case 'fibonacci':
                const fib = this.checkFibonacci(results);
                if (fib.found) {
                    return { prediction: fib.prediction, confidence: 0.7, reason: 'Phát hiện cầu Fibonacci', model_name: model.name };
                }
                break;
            case 'longTrend':
                const trend = this.getLongTrend(results);
                if (trend.strength > 0.7) {
                    return { prediction: trend.direction, confidence: 0.7 + (trend.strength * 0.1), reason: `Xu hướng dài ${trend.direction} với độ mạnh ${(trend.strength*100).toFixed(0)}%`, model_name: model.name };
                }
                break;
            case 'super':
                const superAnalysis = this.superAnalysis(results);
                if (superAnalysis.confidence > 0.8) return { ...superAnalysis, model_name: model.name };
                break;
        }
        return null;
    }
    ...
    analyzeTrend(history) {
        ...
        if (shortTrend.count >= 2 && longTrend.count >= 6) {
            return { prediction: shortTrend.value, confidence: Math.min(0.7 + momentum * 0.1, 0.95), momentum: momentum, reason: `Xu hướng ngắn và dài đều nghiêng về ${shortTrend.value}` };
        } else if (shortTrend.count >= 2) {
            return { prediction: shortTrend.value, confidence: Math.min(0.6 + momentum * 0.1, 0.95), momentum: momentum, reason: `Xu hướng ngắn hạn nghiêng về ${shortTrend.value}` };
        } else if (longTrend.count >= 6) {
            return { prediction: longTrend.value, confidence: Math.min(0.6 + momentum * 0.1, 0.95), momentum: momentum, reason: `Xu hướng dài hạn nghiêng về ${longTrend.value}` };
        } else {
            const other = results[results.length - 1] === 'Tài' ? 'Xỉu' : 'Tài';
            return { prediction: other, confidence: 0.5, momentum: momentum, reason: "Không có trend rõ ràng, dự đoán đảo chiều" };
        }
    }
    ...
    analyzeImbalance(history) {
        ...
        if (imbalanceRatio > 0.4) {
            if (countTai > countXiu) {
                return { prediction: 'Xỉu', confidence: Math.min(0.7 + imbalanceRatio * 0.2, 0.95), tai_count: countTai, xiu_count: countXiu, reason: `Chênh lệch lớn (${countTai}T - ${countXiu}X), dự đoán Xỉu để cân bằng` };
            } else {
                return { prediction: 'Tài', confidence: Math.min(0.7 + imbalanceRatio * 0.2, 0.95), tai_count: countTai, xiu_count: countXiu, reason: `Chênh lệch lớn (${countTai}T - ${countXiu}X), dự đoán Tài để cân bằng` };
            }
        } else {
            return { prediction: results[results.length - 1], confidence: 0.5, tai_count: countTai, xiu_count: countXiu, reason: `Chênh lệch ${countTai}T - ${countXiu}X trong 12 phiên, tiếp tục xu hướng` };
        }
    }
    ...
    analyzeShortTerm(history) {
        ...
        if (patterns.length > 0) {
            ...
            return { prediction: bestPattern.prediction, confidence: bestPattern.confidence, pattern: bestPattern.type, reason: `Phát hiện pattern ${bestPattern.type} trong ngắn hạn` };
        } else {
            return { prediction: results[results.length - 1], confidence: 0.4, pattern: 'không_rõ', reason: "Không phát hiện pattern ngắn hạn rõ ràng" };
        }
    }
    ...
    analyzeDiceVolatility(history) {
        ...
        for (let face = 1; face <= 6; face++) {
            if (recentFreq[face] < 2) predictions.push({ face, prob: 0.3 + (2 - recentFreq[face]) * 0.1 });
        }
        ...
        if (predictions.length > 0) {
            ...
            if (topFaces.length >= 3) {
                ...
                if (predictedScores.length > 0) {
                    ...
                    return { prediction: avgPredicted >= 11 ? 'Tài' : 'Xỉu', confidence: 0.65, predicted_faces: topFaces.map(f => f.face), reason: `Dựa trên biến động xúc xắc, các mặt ${topFaces.map(f => f.face).join(',')} có khả năng xuất hiện cao` };
                }
                ...
                if (predictedScores.length > 0) {
                    const avgPredicted = predictedScores.reduce((a, b) => a + b, 0) / predictedScores.length;
                    return { prediction: avgPredicted >= 11 ? 'Tài' : 'Xỉu', confidence: 0.6, predicted_faces: topFaces.map(f => f.face), reason: `Dựa trên biến động xúc xắc, các mặt ${topFaces.map(f => f.face).join(',')} có khả năng xuất hiện cao` };
                }
            } else {
                const face = topFaces[0].face;
                const avgOther = 3.5;
                const avgPredicted = face + avgOther + avgOther;
                return { prediction: avgPredicted >= 11 ? 'Tài' : 'Xỉu', confidence: 0.55, predicted_faces: [face], reason: `Dựa trên biến động xúc xắc, mặt ${face} có khả năng xuất hiện cao` };
            }
        }
    ...
    ensembleModels(history) {
        ...
        return { prediction: finalPrediction, confidence: finalConfidence, reason: finalReason, details: details.slice(0, 5) };
    }
...
// ==================== CẬP NHẬT LỊCH SỬ TỪ API ====================
let lastProcessedPhien = null;

async function updateFromAPI() {
    try {
        const apiData = await fetchHistoryFromAPI();
        if (!apiData || apiData.length === 0) return;
        
        let newEntries = 0;
        for (const item of apiData) {
            const phien = item.phien || item.session || item.id;
            if (!phien) continue;
            
            const existing = resultHistory.find(h => h.phien === phien);
            if (!existing) {
                const d1 = item.xuc_xac_1 || item.dice1 || item.d1 || 0;
                const d2 = item.xuc_xac_2 || item.dice2 || item.d2 || 0;
                const d3 = item.xuc_xac_3 || item.dice3 || item.d3 || 0;
                const total = item.tong || item.total || (d1 + d2 + d3);
                const result = item.ket_qua || item.result || (total >= 11 ? 'Tài' : 'Xỉu');
        ...
        if (newEntries > 0) {
            resultHistory.sort((a, b) => (a.phien || 0) - (b.phien || 0));
            if (resultHistory.length > 1000) resultHistory = resultHistory.slice(-1000);
            fs.writeFileSync(HISTORY_FILE, JSON.stringify(resultHistory, null, 2));
            console.log(`[📥] Đã thêm ${newEntries} phiên mới. Tổng: ${resultHistory.length}`);
            
            // Xử lý từng phiên mới để cập nhật stats và dự đoán
            for (const newEntry of resultHistory.slice(-newEntries)) {
                await processNewSession(newEntry);
            }
        }
    } catch (e) {
        console.error('[❌] Lỗi update từ API:', e.message);
    }
}

async function processNewSession(session) {
    // Kiểm tra dự đoán cũ
    let predictionCorrect = false;
    if (lastPrediction && lastPrediction.ket_qua) {
        predictionCorrect = (lastPrediction.ket_qua === session.Ket_qua);
        ...
        analyzer.updateModelWeights(session.Ket_qua, lastPrediction.ket_qua, parseFloat(lastPrediction.do_tin_cay));
    }
    
    // Tạo history cho analyzer
    ...
    let finalType = 'TỔNG HỢP ' + (ensembleResult.details.length > 0 ? ensembleResult.details[0].model : '84 MODELS');
    ...
    const nextPhien = session.phien ? parseInt(session.phien) + 1 : (resultHistory.length + 1);
    
    lastPrediction = {
        phien: nextPhien,
        ket_qua: finalPrediction,
        loai_cau: finalType,
        mau_cau: ensembleResult.details.slice(0, 3).map(d => d.model).join(', '),
        do_tin_cay: (finalConfidence * 100).toFixed(0) + '%'
    };
    
    const trangThai = stats.consecutiveLosses >= 3 ? 'Chống đảo' : 'Đang theo cầu';
    const tiLe = stats.total > 0 ? ((stats.correct / stats.total) * 100).toFixed(1) + '%' : '0%';
    
    apiResponseData = {
        "Phien": session.phien,
        "Xuc_xac_1": session.Xuc_xac_1,
        "Xuc_xac_2": session.Xuc_xac_2,
        "Xuc_xac_3": session.Xuc_xac_3,
        "Tong": session.Tong,
        "Ket_qua": session.Ket_qua,
        "Phien_hien_tai": nextPhien,
        "Du_doan": finalPrediction,
        "Loai_cau": finalType,
        "Mau_cau_phat_hien": lastPrediction.mau_cau,
        "Do_tin_cay": (finalConfidence * 100).toFixed(0) + '%',
    ...
    console.log('\n' + '🟦'.repeat(20));
    console.log(`🎲 Phiên ${session.phien} | KQ: ${session.Ket_qua}`);
    console.log(`📊 Lịch sử: ${historyForAnalyzer.slice(-12).map(h => h.Ket_qua).join(' ')}`);
    console.log(`🔍 Phát hiện: ${finalType}`);
...
app.listen(PORT, async () => {
    ...
    console.log(`[🔄] Đang tải lịch sử từ API ${API_URL}...`);
