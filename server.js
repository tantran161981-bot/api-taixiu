...
class TaiXiuAnalyzer {
    ...
    // ==================== CÁC HÀM SUB MODELS (GIỮ NGUYÊN 100%) ====================
...
// ==================== FETCH DATA FROM API (THAY CHO WEBSOCKET) ====================
let currentPhien = null;
let lastProcessedIndex = -1; // Chỉ số của phiên cuối cùng đã xử lý

// Hàm fetch dữ liệu từ API lịch sử
async function fetchHistoryData() {
    try {
        const response = await axios.get('https://apivip-anhkhoi-dzaivcl.onrender.com/data', { timeout: 10000 });
        if (response.data && response.data.data && Array.isArray(response.data.data)) {
            // API trả về dữ liệu mới nhất trước, đảo ngược để xử lý từ cũ đến mới
            const newData = response.data.data.reverse();
            return newData;
        }
        return null;
    } catch (error) {
        console.error('[❌] Lỗi fetch API lịch sử:', error.message);
        return null;
    }
}

// Hàm xử lý phiên mới từ API
async function processNewData() {
    try {
        const allData = await fetchHistoryData();
        if (!allData || allData.length === 0) {
            console.log('[⏳] Chưa có dữ liệu từ API');
            return;
        }

        // Tìm phiên mới nhất chưa xử lý
        let newEntries = [];
        if (lastProcessedIndex === -1) {
            // Lần đầu chạy, xử lý toàn bộ dữ liệu có sẵn
            newEntries = allData;
            lastProcessedIndex = allData.length - 1;
            console.log(`[📥] Lần đầu: tải ${newEntries.length} phiên lịch sử`);
        } else {
            // Tìm các phiên mới hơn so với lastProcessedIndex
            for (let i = lastProcessedIndex + 1; i < allData.length; i++) {
                newEntries.push(allData[i]);
            }
            if (newEntries.length > 0) {
                console.log(`[📥] Phát hiện ${newEntries.length} phiên mới`);
                lastProcessedIndex = allData.length - 1;
            }
        }

        // Xử lý từng phiên mới
        for (const entry of newEntries) {
            const phien = entry.phien;
            const d1 = entry.xuc_xac_1;
            const d2 = entry.xuc_xac_2;
            const d3 = entry.xuc_xac_3;
            const total = entry.tong;
            const result = entry.ket_qua; // "Tài" hoặc "Xỉu"
            ...
            const historyEntry = {
                ...
                thoi_gian: entry.thoi_gian || new Date().toISOString()
            };
            ...
            apiResponseData = {
                ...
                "Tong": total,
                "Ket_qua": result,
                "Phien_hien_tai": phien ? phien + 1 : null,
            ...
            console.log(`🤖 Dự đoán phiên ${phien ? phien + 1 : '?'}: ${finalPrediction} (${(finalConfidence * 100).toFixed(0)}%)`);
        ...
        console.error('[❌] Lỗi xử lý dữ liệu mới:', error.message);
    }
}

// ==================== EXPRESS API (GIỮ NGUYÊN) ====================
...
app.listen(PORT, () => {
    ...
    console.log(`[🔄] Đã chuyển sang dùng API lịch sử: https://apivip-anhkhoi-dzaivcl.onrender.com/data`);
    console.log(`[⏰] Polling dữ liệu mới mỗi 5 giây...`);
    
    // Chạy lần đầu
    setTimeout(() => processNewData(), 2000);
    
    // Polling mỗi 5 giây để lấy dữ liệu mới
    setInterval(() => processNewData(), 5000);
});