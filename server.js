...
function chuyenDoiKetQua(result) {
...
// 7. CẦU 1-2-3 (T - X X - T T T) hoặc (X - T T - X X X)
function cau123(history) {
  if (history.length < 6) return { active: false };
  const last6 = history.slice(-6).map(h => h.result);
  
  // Mẫu: Tài, Xỉu, Xỉu, Tài, Tài, Tài
  if (last6[0] === 'Tài' && last6[1] === 'Xỉu' && last6[2] === 'Xỉu' && 
      last6[3] === 'Tài' && last6[4] === 'Tài' && last6[5] === 'Tài') {
    return { active: true, pred: 'Xiu', doTin: 88, ten: '🏆 CẦU 1-2-3 (T-X X-T T T) → Xỉu' };
  }
  // Mẫu: Xỉu, Tài, Tài, Xỉu, Xỉu, Xỉu
  if (last6[0] === 'Xỉu' && last6[1] === 'Tài' && last6[2] === 'Tài' && 
      last6[3] === 'Xỉu' && last6[4] === 'Xỉu' && last6[5] === 'Xỉu') {
    return { active: true, pred: 'Tai', doTin: 88, ten: '🏆 CẦU 1-2-3 (X-T T-X X X) → Tài' };
  }
  return { active: false };
}

// 8. CẦU 3-2-1 (T T T - X X - T) hoặc (X X X - T T - X)
function cau321(history) {
  if (history.length < 6) return { active: false };
  const last6 = history.slice(-6).map(h => h.result);
  
  // Mẫu: Tài, Tài, Tài, Xỉu, Xỉu, Tài
  if (last6[0] === 'Tài' && last6[1] === 'Tài' && last6[2] === 'Tài' && 
      last6[3] === 'Xỉu' && last6[4] === 'Xỉu' && last6[5] === 'Tài') {
    return { active: true, pred: 'Xiu', doTin: 88, ten: '🏆 CẦU 3-2-1 (T T T-X X-T) → Xỉu' };
  }
  // Mẫu: Xỉu, Xỉu, Xỉu, Tài, Tài, Xỉu
  if (last6[0] === 'Xỉu' && last6[1] === 'Xỉu' && last6[2] === 'Xỉu' && 
      last6[3] === 'Tài' && last6[4] === 'Tài' && last6[5] === 'Xỉu') {
    return { active: true, pred: 'Tai', doTin: 88, ten: '🏆 CẦU 3-2-1 (X X X-T T-X) → Tài' };
  }
  return { active: false };
}

// 9. CẦU 1-2-1 (T - X X - T) hoặc (X - T T - X)
function cau121(history) {
  if (history.length < 5) return { active: false };
  const last5 = history.slice(-5).map(h => h.result);
  
  // Tài, Xỉu, Xỉu, Tài, ? -> dự đoán Xỉu (theo cầu 1-2-1)
  if (last5[0] === 'Tài' && last5[1] === 'Xỉu' && last5[2] === 'Xỉu' && last5[3] === 'Tài') {
    return { active: true, pred: 'Xiu', doTin: 86, ten: '📊 CẦU 1-2-1 (T-X X-T) → Xỉu' };
  }
  // Xỉu, Tài, Tài, Xỉu, ? -> dự đoán Tài
  if (last5[0] === 'Xỉu' && last5[1] === 'Tài' && last5[2] === 'Tài' && last5[3] === 'Xỉu') {
    return { active: true, pred: 'Tai', doTin: 86, ten: '📊 CẦU 1-2-1 (X-T T-X) → Tài' };
  }
  return { active: false };
}

// 10. CẦU 2-1-2 (T T - X - T T) hoặc (X X - T - X X)
function cau212(history) {
  if (history.length < 5) return { active: false };
  const last5 = history.slice(-5).map(h => h.result);
  
  // Tài, Tài, Xỉu, Tài, Tài -> dự đoán Xỉu
  if (last5[0] === 'Tài' && last5[1] === 'Tài' && last5[2] === 'Xỉu' && last5[3] === 'Tài' && last5[4] === 'Tài') {
    return { active: true, pred: 'Xiu', doTin: 87, ten: '🎯 CẦU 2-1-2 (T T-X-T T) → Xỉu' };
  }
  // Xỉu, Xỉu, Tài, Xỉu, Xỉu -> dự đoán Tài
  if (last5[0] === 'Xỉu' && last5[1] === 'Xỉu' && last5[2] === 'Tài' && last5[3] === 'Xỉu' && last5[4] === 'Xỉu') {
    return { active: true, pred: 'Tai', doTin: 87, ten: '🎯 CẦU 2-1-2 (X X-T-X X) → Tài' };
  }
  return { active: false };
}

// 11. CẦU 1-3-2 (T - X X X - T T) hoặc (X - T T T - X X)
function cau132(history) {
  if (history.length < 6) return { active: false };
  const last6 = history.slice(-6).map(h => h.result);
  
  // Tài, Xỉu, Xỉu, Xỉu, Tài, Tài -> dự đoán Xỉu
  if (last6[0] === 'Tài' && last6[1] === 'Xỉu' && last6[2] === 'Xỉu' && last6[3] === 'Xỉu' && 
      last6[4] === 'Tài' && last6[5] === 'Tài') {
    return { active: true, pred: 'Xiu', doTin: 86, ten: '🎯 CẦU 1-3-2 (T-X X X-T T) → Xỉu' };
  }
  // Xỉu, Tài, Tài, Tài, Xỉu, Xỉu -> dự đoán Tài
  if (last6[0] === 'Xỉu' && last6[1] === 'Tài' && last6[2] === 'Tài' && last6[3] === 'Tài' && 
      last6[4] === 'Xỉu' && last6[5] === 'Xỉu') {
    return { active: true, pred: 'Tai', doTin: 86, ten: '🎯 CẦU 1-3-2 (X-T T T-X X) → Tài' };
  }
  return { active: false };
}

// 12. CẦU 2-3-1 (T T - X X X - T) hoặc (X X - T T T - X)
function cau231(history) {
  if (history.length < 6) return { active: false };
  const last6 = history.slice(-6).map(h => h.result);
  
  // Tài, Tài, Xỉu, Xỉu, Xỉu, Tài -> dự đoán Xỉu
  if (last6[0] === 'Tài' && last6[1] === 'Tài' && last6[2] === 'Xỉu' && last6[3] === 'Xỉu' && 
      last6[4] === 'Xỉu' && last6[5] === 'Tài') {
    return { active: true, pred: 'Xiu', doTin: 87, ten: '🎯 CẦU 2-3-1 (T T-X X X-T) → Xỉu' };
  }
  // Xỉu, Xỉu, Tài, Tài, Tài, Xỉu -> dự đoán Tài
  if (last6[0] === 'Xỉu' && last6[1] === 'Xỉu' && last6[2] === 'Tài' && last6[3] === 'Tài' && 
      last6[4] === 'Tài' && last6[5] === 'Xỉu') {
    return { active: true, pred: 'Tai', doTin: 87, ten: '🎯 CẦU 2-3-1 (X X-T T T-X) → Tài' };
  }
  return { active: false };
}

// 13. CẦU 1-1-2 (T - X - T T) hoặc (X - T - X X)
function cau112(history) {
  if (history.length < 4) return { active: false };
  const last4 = history.slice(-4).map(h => h.result);
  
  if (last4[0] === 'Tài' && last4[1] === 'Xỉu' && last4[2] === 'Tài' && last4[3] === 'Tài') {
    return { active: true, pred: 'Xiu', doTin: 83, ten: '📌 CẦU 1-1-2 (T-X-T T) → Xỉu' };
  }
  if (last4[0] === 'Xỉu' && last4[1] === 'Tài' && last4[2] === 'Xỉu' && last4[3] === 'Xỉu') {
    return { active: true, pred: 'Tai', doTin: 83, ten: '📌 CẦU 1-1-2 (X-T-X X) → Tài' };
  }
  return { active: false };
}
...
function cau3Phien(history) {
  ...
  const patterns = {
    ...
    'Tài,Tài,Tài': { pred: 'Xiu', doTin: 90, ten: '🔥 TTT -> X' },
    'Xỉu,Xỉu,Xỉu': { pred: 'Tai', doTin: 90, ten: '🔥 XXX -> T' }
  };
...
function cauDaoChieu(history) {
  ...
  if (changes >= 4) {
    return { active: true, pred: last6[5] === 'Tài' ? 'Xiu' : 'Tai', doTin: 84, ten: `🔄 ĐẢO CHIỀU (${changes}/5)` };
  }
  return { active: false };
}

// 19. CẦU THEO TỔNG (chẵn/lẻ)
...
function cauGanh(history) {
  ...
  if (last5[0] === last5[2] && last5[2] === last5[4] && last5[0] !== last5[1]) {
    return { active: true, pred: last5[4] === 'Tài' ? 'Xiu' : 'Tai', doTin: 83, ten: '⚖️ CẦU GÁNH' };
  }
  return { active: false };
}
...
function cauDoiXung(history) {
  ...
  if (last6[0] === last6[5] && last6[1] === last6[4] && last6[2] === last6[3]) {
    return { active: true, pred: last6[5] === 'Tài' ? 'Xiu' : 'Tai', doTin: 82, ten: '🪞 CẦU ĐỐI XỨNG' };
  }
  return { active: false };
}

// 23. KIỂM TRA 3 PHIÊN GIỐNG NHAU
function cau3Lien(history) {
  if (history.length < 3) return { active: false };
  const last3 = history.slice(-3).map(h => h.result);
  if (last3[0] === last3[1] && last3[1] === last3[2]) {
    return { active: true, pred: last3[0] === 'Tài' ? 'Xiu' : 'Tai', doTin: 88, ten: '⚡ BẺ 3 PHIÊN GIỐNG' };
  }
  return { active: false };
}

// 24. CÂN BẰNG TẦN SUẤT
...
function duDoanTongHop(history) {
  ...
  const cacCau = [
    cauBet(history),
    cau11(history), cau22(history), cau33(history), cau44(history),
    cauZigzag(history), cauTamGiac(history),
    cau123(history), cau321(history), cau121(history), cau212(history),
    cau132(history), cau231(history), cau112(history),
    cauLap2(history), cauLap3(history),
    cau3Phien(history), cauDaoChieu(history),
    cauGanh(history), cauDoiXung(history), cau3Lien(history),
    cauCanBangTanSuat(history),
    cauTongChanLe(history), cauTongTangGiam(history)
  ];
  ...
  let duDoanCuoi = diemTai > diemXiu ? 'Tai' : 'Xiu';
  let doTinCuoi = Math.min(92, Math.max(65, Math.floor((Math.max(diemTai, diemXiu) / tongDiem) * 100)));
...
app.get('/', (req, res) => {
  res.send('API Tài Xỉu - 30+ mẫu cầu | Bẻ khi bệt >=7');
});
...
app.listen(PORT, () => {
  ...
  console.log(`📌 30+ mẫu cầu: Bệt, 1-1, 2-2, 3-3, 4-4, Zigzag, Tam giác,`);
  console.log(`   1-2-3, 3-2-1, 1-2-1, 2-1-2, 1-3-2, 2-3-1, 1-1-2,`);
  console.log(`   Lặp, 3 phiên, Đảo chiều, Gánh, Đối xứng, Tổng...`);
});
